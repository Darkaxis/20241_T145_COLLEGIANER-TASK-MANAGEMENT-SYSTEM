document.addEventListener('DOMContentLoaded', async function() {
    const response = await fetch('https://localhost:3000/api/v1/staff/tasks/archives', {
        method: 'GET',
        headers: {
    ...
    async function restoreItem(task, element) {
        ...
        modal.querySelector('.confirm-btn').onclick = async () => {
            ...
            const response = await fetch(`https://localhost:3000/api/v1/staff/tasks/unarchive/${task.id}`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json'
                },
